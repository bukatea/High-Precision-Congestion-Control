	...
	void RdmaHw::Te_timeout() {
		...
		for (uint32_t i = 0; i < IntHeader::maxHop; i++) {
			...
			if (m_qp->xcpint.hopState[i].m_valid) {
				...
				//double pos_fbk = residue_pos_fbk * m_qp->xcpint.m_packet_size / (m_qp->xcpint.m_curRate.GetBitRate() / 8.0 * m_avg_rtt * (num_active_flows - m_qp->xcpint.hopState[i].m_start_num_flows));
				//double neg_fbk = residue_neg_fbk * m_qp->xcpint.m_packet_size / rv;
				double pos_fbk = residue_pos_fbk / (num_active_flows - m_qp->xcpint.hopState[i].m_start_num_flows);
