	...
	if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
	  	...
	  	if (IntHeader::mode == 20) {
	  		...
	  		i.WriteHtonU64(ui);
	  		std::memset(&ui, &udp.reverse_concflows_inc_sum, sizeof(double));
	  		i.WriteHtonU64(ui);
	  	...
	  	i.WriteU16(ack.pg);
	  	i.WriteU32(ack.seq);
        // might need to change ack to udp here but don't know yet
	  	if (IntHeader::mode == 20) {
	  		i.WriteHtonU64(ack.ts);
	  		uint64_t ui;
	  		std::memset(&ui, &ack.concflows_inc, sizeof(double));
	  		i.WriteHtonU64(ui);
	  		std::memset(&ui, &ack.reverse_concflows_inc_sum, sizeof(double));
	  		i.WriteHtonU64(ui);
	  		i.WriteHtonU32(ack.xcpId);
	  	}
	  	udp.ih.Serialize(i);
	...
	if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
	  	...
	  	if (IntHeader::mode == 20) {
	  		...
	  		ui = i.ReadNtohU64();
	  		std::memset(&udp.reverse_concflows_inc_sum, &ui, sizeof(double));
	  	...
	  	ack.seq = i.ReadU32();
	  	if (IntHeader::mode == 20) {
	  		ack.ts = i.ReadNtohU64();
	  		uint64_t ui;
	  		ui = i.ReadNtohU64();
	  		std::memset(&ack.concflows_inc, &ui, sizeof(double));
	  		ui = i.ReadNtohU64();
	  		std::memset(&ack.reverse_concflows_inc_sum, &ui, sizeof(double));
...
uint32_t CustomHeader::GetAckSerializedSize(void){
	...
	return IntHeader::mode == 20 ? val + sizeof(ack.ts) + sizeof(ack.concflows_inc) + sizeof(ack.reverse_concflows_inc_sum) + sizeof(ack.xcpId) : val;
}

uint32_t CustomHeader::GetUdpHeaderSize(void){
	uint32_t val = 8 + sizeof(udp.pg) + sizeof(udp.seq) + IntHeader::GetStaticSize();
	return IntHeader::mode == 20 ? val + sizeof(udp.ts) + sizeof(udp.concflows_inc) + sizeof(udp.reverse_concflows_inc_sum) + sizeof(udp.xcpId) : val;
}
